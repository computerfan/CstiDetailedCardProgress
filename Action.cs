...
namespace CstiDetailedCardProgress
{
    class Action
    {
        ...
        public static string FormatCardDropList(CollectionDropReport report, InGameCardBase fromCard, bool withStat = true,bool withCard = true, bool withDuability = true)
        {
            ...
            for(int i = 0; i < report.DropsInfo.Length; i++)
            {
                ...
                texts.Add(FormatBasicEntry($"{dropRate:P2}", $"{report.DropsInfo[i].CollectionName}"));
                texts.Add(FormatBasicEntry($"{report.DropsInfo[i].FinalWeight}/{report.TotalValue}", "Weight", indent: 2));
                if (report.DropsInfo[i].BaseWeight != 0)
                {
                    texts.Add(FormatTooltipEntry(report.DropsInfo[i].BaseWeight, "Base", 4));
                }
                if (withStat && report.DropsInfo[i].StatMods != null)
                {
                    report.DropsInfo[i].StatWeightMods.Do(statmod =>
                    {
                        if(statmod.BonusWeight!=0) texts.Add(FormatTooltipEntry(statmod.BonusWeight, $"{statmod.Stat.GameName}", 4));
                    });
                }
                if(withCard && report.DropsInfo[i].CardWeightMods != null)
                {
                    ...
                    {
                        if(cardmod.BonusWeight!=0) texts.Add(FormatTooltipEntry(cardmod.BonusWeight, $"{cardmod.Card.CardModel.CardName}", 4));
                    });
                }
                if (withDuability)
                {
                    if (report.DropsInfo[i].DurabilitiesWeightMods.SpoilageWeight != 0)
                        texts.Add(FormatTooltipEntry(report.DropsInfo[i].DurabilitiesWeightMods.SpoilageWeight, $"{fromCard.CardModel.SpoilageTime.CardStatName}", 4));
                    if(report.DropsInfo[i].DurabilitiesWeightMods.UsageWeight!=0)
                        texts.Add(FormatTooltipEntry(report.DropsInfo[i].DurabilitiesWeightMods.UsageWeight, $"{fromCard.CardModel.UsageDurability.CardStatName}", 4));
                    if(report.DropsInfo[i].DurabilitiesWeightMods.FuelWeight!=0)
                        texts.Add(FormatTooltipEntry(report.DropsInfo[i].DurabilitiesWeightMods.FuelWeight, $"{fromCard.CardModel.FuelCapacity.CardStatName}", 4));
                    if(report.DropsInfo[i].DurabilitiesWeightMods.ProgressWeight!=0)
                        texts.Add(FormatTooltipEntry(report.DropsInfo[i].DurabilitiesWeightMods.ProgressWeight, $"{fromCard.CardModel.Progress.CardStatName}", 4));
                    if(report.DropsInfo[i].DurabilitiesWeightMods.Special1Weight!=0)
                        texts.Add(FormatTooltipEntry(report.DropsInfo[i].DurabilitiesWeightMods.Special1Weight, $"{fromCard.CardModel.SpecialDurability1.CardStatName}", 4));
                    if(report.DropsInfo[i].DurabilitiesWeightMods.Special2Weight!=0)
                        texts.Add(FormatTooltipEntry(report.DropsInfo[i].DurabilitiesWeightMods.Special2Weight, $"{fromCard.CardModel.SpecialDurability2.CardStatName}", 4));
                    if (report.DropsInfo[i].DurabilitiesWeightMods.Special3Weight != 0)
                        texts.Add(FormatTooltipEntry(report.DropsInfo[i].DurabilitiesWeightMods.Special3Weight, $"{fromCard.CardModel.SpecialDurability3.CardStatName}", 4));
                    if (report.DropsInfo[i].DurabilitiesWeightMods.Special4Weight != 0)
                        texts.Add(FormatTooltipEntry(report.DropsInfo[i].DurabilitiesWeightMods.Special4Weight, $"{fromCard.CardModel.SpecialDurability4.CardStatName}", 4));
                }
            }
